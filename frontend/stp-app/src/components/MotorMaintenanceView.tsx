          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: '13px' }}>
            ...
            <tbody>
              {allMotors.map(({ motor, tankName, telemetryMs }, idx) => {
                ...
                const totalRunHours = parseInt(lastLog.running_hours || '1240');

                // 1. Grease / Bearing Check Calculation (Every 2,000 hours or 90 days)
                const greaseHoursLeft = Math.max(0, 2000 - (totalRunHours % 2000));
                const greaseDaysLeft = Math.round(greaseHoursLeft / 16); // ~16 operating hours/day

                // 2. Full Service / Overhaul Calculation (Every 5,000 hours or 365 days)
                const overhaulHoursLeft = Math.max(0, 5000 - (totalRunHours % 5000));
